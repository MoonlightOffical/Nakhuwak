...
NAKHUN HUB NETA EDITION SCRIPT

This made by Nakhun Hub Team ( io.Luable / Nakhun Hub )
Modification of the script, including attempting to bypass
or crack the script for any reason is not allowed.

Copyright © 2025 Nakhun Hub - Script. All Rights Reserved.
        ...
        game.DescendantAdded:Connect(function(descendant)
            if descendant.ClassName == "TextLabel" then
                if descendant.Text == n_protect then
                    descendant.Text = "Nakhun Hub"
                    descendant.Changed:Connect(function()
                        descendant.Text = "Nakhun Hub"
                    end)
                elseif descendant.Text == "@"..f_protect then
                    descendant.Text = "@Protect By Nakhun"
                    descendant.Changed:Connect(function()
                        descendant.Text = "@Protect By Nakhun"
                    end)
                end
	...
	Buttons = {
		{
			Title = "Sure!",
			Callback = function()
				print('discord.gg/hrGTJuh9et')
                if setclipboard then
                    setclipboard("Dont for get to join > https://discord.gg/hrGTJuh9et")
                end
			end,
		},
		{
